/*$Id: rspvk_m.c,v 1.18 2011-02-21 07:35:57 sasa Exp $*/
/*25.02.2009	28.03.2004	Белых А.И.	rspvk_m.c
  ...
  void read_rek()
   {
    ...
    rek_r->kontr.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_KONTR]))));
   }
...
iceb_u_str rspvk_rr::datan;
iceb_u_str rspvk_rr::datak;
iceb_u_str rspvk_rr::shet;
iceb_u_str rspvk_rr::kodgr;
iceb_u_str rspvk_rr::kontr;
...
sprintf(strsql,"%s %s",name_system,gettext("Сальдо"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(rspvk_v_key_press),&data);

GtkWidget *label=gtk_label_new(gettext("Расчет сальдо по всем контрагентам"));
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAN]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAN]), "activate",GTK_SIGNAL_FUNC(rspvk_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAK]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAK]), "activate",GTK_SIGNAL_FUNC(rspvk_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_SHET]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_SHET]), "activate",GTK_SIGNAL_FUNC(rspvk_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KODGR]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KODGR]), "activate",GTK_SIGNAL_FUNC(rspvk_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KONTR]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KONTR]), "activate",GTK_SIGNAL_FUNC(rspvk_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(rspvk_v_knopka),&data);
