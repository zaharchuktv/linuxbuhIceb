/*$Id: iceb_l_opmur.c,v 1.34 2011-02-21 07:36:07 sasa Exp $*/
/*08.11.2009	05.05.2004	Белых А.И.	iceb_l_opmur.c
Работа со списком операций расхода для материального учёта
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gboolean   opmur_key_press(GtkWidget *widget,GdkEventKey *event,class opmur_data *data);
void opmur_vibor(GtkTreeSelection *selection,class opmur_data *data);
void opmur_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class opmur_data *data);
void  opmur_knopka(GtkWidget *widget,class opmur_data *data);
void opmur_add_columns (GtkTreeView *treeview);
int iceb_l_opmur_v(class iceb_u_str*,GtkWidget*);
int iceb_l_opmur_p(class opmur_rek *data,GtkWidget*);
void opmur_udzap(class opmur_data *data);
int  opmur_prov_row(SQL_str row,class opmur_data *data);
void opmur_rasp(class opmur_data *data);
int  opmur_pvu(int,const char *kod,GtkWidget *wpredok);
void opmur_create_list (class opmur_data *data);

extern char *organ;

int   iceb_l_opmur(int metka_rr, //0-ввод и корек. 1-выбор
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(opmur_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(opmur_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(opmur_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(opmur_vibor),data);
...
opmur_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список операций расходов"));
...
//g_print("opmur_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    iceb_l_opmur_v(&data->kod_tv,data->window);
    opmur_create_list(data);
    return;  
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    iceb_l_opmur_v(&data->kodv,data->window);
    opmur_create_list(data);
    return;  
  ...
  case FK4:
    if(iceb_l_opmur_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    opmur_create_list(data);
...
{
iceb_u_str repl;
//printf("opmur_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
switch(event->keyval)
 {
  ...
  case ICEB_REG_L:
  case ICEB_REG_R:
    printf("opmur_key_press-Нажата клавиша Shift\n");
...
{
//printf("opmur_v_row\n");
//printf("opmur_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
if(opmur_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Rashod where kod='%s'",
data->kodv.ravno());
...
sprintf(strsql,"mopr%d.lst",getpid());
...
int  opmur_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kodop from Dokummat where kodop='%s' limit 1",kod);
...
sprintf(strsql,"select ko from Reenn where ko='%s' limit 1",kod);
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
