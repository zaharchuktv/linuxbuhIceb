/*$Id: l_uslugi_p.c,v 1.13 2011-02-21 07:35:54 sasa Exp $*/
/*20.07.2005	18.07.2005	Белых А.И.	l_uslugi_p.c
...
gboolean   uslugi_p_key_press(GtkWidget *widget,GdkEventKey *event,class uslugi_p_data *data);
void    uslugi_p_vvod(GtkWidget *widget,class uslugi_p_data *data);
void  uslugi_p_knopka(GtkWidget *widget,class uslugi_p_data *data);
void  uslugi_v_e_knopka(GtkWidget *widget,class uslugi_p_data *data);
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(uslugi_p_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(uslugi_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(uslugi_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_GRUPA]), "activate",GTK_SIGNAL_FUNC(uslugi_p_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_GRUPA]),data.rpoi->grupa.ravno_toutf());
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(uslugi_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(uslugi_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(uslugi_p_knopka),&data);
...
switch (knop)
 {
  case E_GRUPA:
    if(l_uslgrup(1,&kod,&naim,data->window) == 0)
     data->rpoi->grupa.z_plus(kod.ravno());
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_GRUPA]),data->rpoi->grupa.ravno_toutf());
    return;
 }
}
...
switch (enter)
 {
  ...
  case E_GRUPA:
    data->rpoi->grupa.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
