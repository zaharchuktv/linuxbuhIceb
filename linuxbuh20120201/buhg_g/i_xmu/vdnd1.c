...
sprintf(strsql,"%s",gettext("Номер счет-фактуры"));
label=gtk_label_new(iceb_u_toutf(strsql));
...
sprintf(strsql,"%s",gettext("Дата получения счет-фактуры"));
...
if(data->rk.nomnalnak.getdlinna() > 1 && data->rk.data_pnn.getdlinna() <= 1)
 {
  repl.new_plus(gettext("Не введена дата получения счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
...
if(iceb_u_rsdat(&d,&m,&g,data->rk.data_pnn.ravno(),0) != 0)
 {
  repl.new_plus(gettext("Не верно введена дата получения счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
 }
...
//Проверяем дату получения счет-фактуры
if(data->rk.data_pnn.prov_dat() != 0)
 {
  repl.new_plus(gettext("Не правильно введена дата получения счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
 }
