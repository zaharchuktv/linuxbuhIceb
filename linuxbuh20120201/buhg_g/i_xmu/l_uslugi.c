/*$Id: l_uslugi.c,v 1.35 2011-02-21 07:35:54 sasa Exp $*/
/*16.07.2009	17.07.2005	Белых А.И.	l_uslugi.c
...
sprintf(bros,"%s %s",name_system,gettext("Список услуг"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(uslugi_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
tooltips[SFK3]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips[SFK3],data.knopka[SFK3],gettext("Удалить неиспользуемые коды услуг"),NULL);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(uslugi_knopka),&data);
...
{
iceb_u_str repl;
//printf("uslugi_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Код группы услуги
  ss[COL_KODGR].new_plus(iceb_u_toutf(row[1]));
  ...
  //Цена
  ss[COL_CENA].new_plus(iceb_u_toutf(row[5]));

  //Метка цены
  if(atoi(row[6]) == 0)
    ss[COL_NDS].new_plus(gettext("Без НДС"));
  else
    ss[COL_NDS].new_plus(gettext("С НДС"));
...
{
//printf("uslugi_v_row\n");
//printf("uslugi_v_row корректировка\n");
...
sprintf(strsql,"usl%d.lst",getpid());
...
iceb_u_zagolov(gettext("Список услуг"),0,0,0,0,0,0,organ,ff);
...
while(cur.read_cursor(&row) != 0)
 {
  if(uslugi_prov_row(row,data) != 0)
    continue;
  if(atoi(row[6]) == 1)
   strncpy(nds,gettext("С НДС"),sizeof(nds)-1);
  else
   strncpy(nds,gettext("Без НДС"),sizeof(nds)-1);
