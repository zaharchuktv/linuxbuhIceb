/*$Id: l_prov_kr.c,v 1.30 2011-01-13 13:49:50 sasa Exp $*/
/*29.09.2009	13.01.2004	Белых А.И.	l_prov_kr.c
...
int l_prov_kr(class prov_data *wdata)
{
char strsql[512];
iceb_u_str stroka;
prov_kr_data data;
...
if(wdata->window != NULL)
 {
  gdk_window_set_cursor(wdata->window->window,gdk_cursor_new(ICEB_CURSOR_GDITE));
...
gtk_signal_connect_after(GTK_OBJECT(data.kwindow),"key_press_event",GTK_SIGNAL_FUNC(prov_kr_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KSHET]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
data.entry[E_KSHETK] = gtk_entry_new_with_max_length (19);
gtk_box_pack_start (GTK_BOX (hbox2), label, FALSE, FALSE, 0);
gtk_box_pack_start (GTK_BOX (hbox2), data.entry[E_KSHETK], TRUE, TRUE, 0);
gtk_signal_connect(GTK_OBJECT (data.entry[E_KSHETK]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KSHETK]),data.rek_prov_s->shetk.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_KSHETK]),(gpointer)E_KSHETK);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KDATA]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KKOMENT]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KDEBET]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KKREDIT]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KKONTR]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KEKV]), "activate",GTK_SIGNAL_FUNC(prov_kr_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KEKV]),data.rek_prov_s->kekv.ravno_toutf());
...
gtk_signal_connect(GTK_OBJECT(data.knopka[KFK2]),"clicked",GTK_SIGNAL_FUNC(prov_kr_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[KFK3]),"clicked",GTK_SIGNAL_FUNC(prov_kr_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[KFK10]),"clicked",GTK_SIGNAL_FUNC(prov_kr_knopka),&data);
...
if(wdata->window != NULL)
  gdk_window_set_cursor(wdata->window->window,gdk_cursor_new(ICEB_CURSOR));
...
switch (enter)
 {
  ...
  case E_KSHETK:
    data->rek_prov_n.shetk.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
  ...
  case E_KEKV:
    data->rek_prov_n.kekv.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
...
if(data->rek_prov_n.shetk.ravno()[0] == '\0')
 {
  iceb_u_str SOOB;
  SOOB.plus(gettext("Не введен кореспондирующий счет !"));
  iceb_menu_soob(&SOOB,data->kwindow);
  return(1);  
 }
...
/*Проверяем есть ли счет корреспондент в списке счетов*/
OPSHET rek_shetk;
if(iceb_prsh1(data->rek_prov_n.shetk.ravno(),&rek_shetk,data->kwindow) != 0)
  return(1);

if(rek_shetk.saldo == 3 && kontr1[0] == '\0')
 {
  iceb_u_str SOOB;
  sprintf(strsql,"%s %s !",gettext("Не введен контрагент для счета"),data->rek_prov_n.shetk.ravno());
  SOOB.plus(strsql);
  iceb_menu_soob(&SOOB,data->kwindow);
  return(1);
 }
...
if(kontr1[0] != '\0')
 {
  sprintf(strsql,"select kodkon from Skontr where ns='%s' and \
kodkon='%s'",data->rek_prov_n.shetk.ravno(),kontr1);
  if(sql_readkey(&bd,strsql) != 1)
   {
    iceb_u_str SOOB;
    sprintf(strsql,"%s %s %s %s!",
...
if(rek_shetk.saldo != rek_shet.saldo && iceb_u_SRAV(kontr,kontr1,0) != 0)
 {  
  iceb_u_str SOOB;
  SOOB.plus(gettext("Не верно введён код контрагента !"));
  iceb_menu_soob(&SOOB,data->kwindow);
  return(1);
 }

if((rek_shetk.saldo == 3 || rek_shet.saldo == 3) && (kontr[0] == '\0' || kontr1[0] == '\0'))
 {  
  iceb_u_str SOOB;
...
if((rek_shetk.saldo < 3 && rek_shet.saldo < 3) && (kontr[0] != '\0' || kontr1[0] != '\0'))
 {  
  iceb_u_str SOOB;
  SOOB.plus(gettext("Оба счета со свернутым сальдо ! Код контрагента не вводится !"));
  iceb_menu_soob(&SOOB,data->kwindow);
  return(1);
 }
