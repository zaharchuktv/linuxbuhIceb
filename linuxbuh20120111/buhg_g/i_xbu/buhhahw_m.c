/*$Id: buhhahw_m.c,v 1.12 2011-02-21 07:35:51 sasa Exp $*/
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(buhhahw_m_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAN]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAN]), "activate",GTK_SIGNAL_FUNC(buhhahw_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAK]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAK]), "activate",GTK_SIGNAL_FUNC(buhhahw_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_DATAK]),data.rk->variant.ravno_toutf());
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_VARIANT]), "activate",GTK_SIGNAL_FUNC(buhhahw_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_VARIANT]),data.rk->datak.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_VARIANT]),(gpointer)E_VARIANT);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(buhhahw_m_knopka),&data);
...
switch (enter)
 {
  ...
  case E_DATAK:
    ...
    break;
  case E_VARIANT:
    data->rk->variant.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
 }
enter+=1;
if(enter >= KOLENTER)
