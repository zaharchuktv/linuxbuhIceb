/*$Id: l_prov_kas.c,v 1.25 2011-02-21 07:35:53 sasa Exp $*/
/*18.11.2008	03.11.2005	Белых А.И.	l_prov_kas.c
...
class prov_kas_data
 {
  ...
  iceb_u_str shetd; //Счет документа            
...
int tipz,     //1-приход 2-расход
const char *shetd, //Счет документа
GtkWidget *wpredok)
{
class prov_kas_data data;
char strsql[400];
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(prov_kas_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[0]), "clicked",GTK_SIGNAL_FUNC(prov_kas_radio0),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[1]), "clicked",GTK_SIGNAL_FUNC(prov_kas_radio1),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[2]), "clicked",GTK_SIGNAL_FUNC(prov_kas_radio2),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK7]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(prov_kas_knopka),&data);
...
datd='%04d-%02d-%02d' and oper='%s' and tz=%d order by datp asc",
  data->metkasys,data->kassa.ravno_atoi(),data->nomdok.ravno_filtr(),
...
switch (knop)
 {
  ...
  case FK2:
    if(prov_kas_prpvblok(data) != 0)
     return;
    if(iceb_vprov(data->metkasys,"","","","","",data->nomdok.ravno(),data->kodop.ravno(),data->kassa.ravno_atoi(),
    data->dd,data->md,data->gd,"",data->vrem_v,data->ktoz_v,"",
    7,data->tipz,0,0,data->window) != 0)
         return;  
  ...
  case SFK2:
    ...
    data->dd,data->md,data->gd,"",data->vrem_v,data->ktoz_v,"",
    7,data->tipz,data->val_v,data->kekv,data->window) != 0)
         return;  
  ...
  case SFK3:
    ...
    if(iceb_menu_danet(&shetd,2,data->window) != 1)
     return;
    if(iceb_udprgr(data->metkasys,data->dd,data->md,data->gd,data->nomdok.ravno(),data->kassa.ravno_atoi(),data->tipz,data->window) != 0)
     return;
