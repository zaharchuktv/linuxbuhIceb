/*$Id: l_otrvr_v.c,v 1.14 2011-02-21 07:35:53 sasa Exp $*/
/*26.11.2008	08.09.2006	Белых А.И.	l_otrvr_v.c
  ...
  void read_rek()
   {
    if(tabnom.getdlinna() <= 1)
     {
      tabnom_v.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_TABNOM]))));
      kod_v.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_KOD_TAB]))));
     }
...
int metka_no=0;
dolhn1w(data.tabnom.ravno_atoi(),&data.oklad,&data.has1,&data.kof,&data.met,&data.koddo,&data.metka,&data.metka1,data.shet_do,&metka_no,wpredok);
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_otrvr_v_v_key_press),&data);
...
if(data.tabnom.getdlinna() <= 1)
 {
  ...
  gtk_signal_connect(GTK_OBJECT (data.entry[E_TABNOM]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
  sprintf(strsql,"%s %s",data.tabnom_v.ravno(),fio);
  gtk_entry_set_text(GTK_ENTRY(data.entry[E_TABNOM]),iceb_u_toutf(strsql));
  ...
  gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD_TAB]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
  sprintf(strsql,"%s %s",data.kod_v.ravno(),naim_tab);
  gtk_entry_set_text(GTK_ENTRY(data.entry[E_KOD_TAB]),iceb_u_toutf(strsql));
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOL_OTR_DNEI]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOL_OTR_HAS]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOL_KAL_DNEI]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOL_RAB_DNEI]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOL_RAB_HAS]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAN]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAK]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOMENT]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NOM_ZAP]), "activate",GTK_SIGNAL_FUNC(l_otrvr_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_otrvr_v_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_otrvr_v_v_knopka),&data);
...
switch (knop)
 {
  ...
  case E_TABNOM:
    ...
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_TABNOM]),data->tabnom_v.ravno_toutf());
    return;  
  ...
  case E_KOD_TAB:
    ...
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KOD_TAB]),data->kod_v.ravno_toutf());
    return;  
 }
}
...
switch (enter)
 {
  ...
  case E_TABNOM:
    data->tabnom_v.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
  ...
  case E_KOD_TAB:
    data->kod_v.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
