/*$Id: zar_soc_nah_m.c,v 1.9 2011-02-21 07:35:59 sasa Exp $*/
/*16.01.2007	16.01.2007	Белых А.И.	zar_soc_nah_m.c
  ...
  void read_rek()
   {
    ...
    rk->kod_soc.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_KOD_SOC]))));
...
sprintf(strsql,"%s %s",name_system,gettext("Распечатать расчёт соц. отчислений на фонд оплаты труда"));
label=gtk_label_new(gettext("Распечатать расчёт соц. отчислений на фонд оплаты труда"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
gtk_signal_connect(GTK_OBJECT(data.window),"delete_event",GTK_SIGNAL_FUNC(gtk_widget_destroy),NULL);
gtk_signal_connect(GTK_OBJECT(data.window),"destroy",GTK_SIGNAL_FUNC(gtk_main_quit),NULL);
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(zar_soc_nah_m_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAN]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAN]), "activate",GTK_SIGNAL_FUNC(zar_soc_nah_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAK]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAK]), "activate",GTK_SIGNAL_FUNC(zar_soc_nah_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_TABNOM]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_TABNOM]), "activate",GTK_SIGNAL_FUNC(zar_soc_nah_m_vvod),&data);
...
data.knopka_enter[E_KOD_SOC]=gtk_button_new_with_label(iceb_u_toutf(strsql));
gtk_box_pack_start (GTK_BOX (hbox[E_KOD_SOC]), data.knopka_enter[E_KOD_SOC], FALSE, FALSE, 0);
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KOD_SOC]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_e_knopka),&data);
gtk_object_set_user_data(GTK_OBJECT(data.knopka_enter[E_KOD_SOC]),(gpointer)E_KOD_SOC);
tooltips_enter[E_KOD_SOC]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips_enter[E_KOD_SOC],data.knopka_enter[E_KOD_SOC],gettext("Выбор кода соц. фонда"),NULL);

data.entry[E_KOD_SOC] = gtk_entry_new();
gtk_box_pack_start (GTK_BOX (hbox[E_KOD_SOC]), data.entry[E_KOD_SOC], TRUE, TRUE, 0);
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD_SOC]), "activate",GTK_SIGNAL_FUNC(zar_soc_nah_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KOD_SOC]),data.rk->kod_soc.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_KOD_SOC]),(gpointer)E_KOD_SOC);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_PODR]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_PODR]), "activate",GTK_SIGNAL_FUNC(zar_soc_nah_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(zar_soc_nah_m_knopka),&data);
...
switch (enter)
 {
  ...
  case E_PODR:
    ...
    break;
  case E_KOD_SOC:
    data->rk->kod_soc.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
