/*$Id: l_uplpod_v.c,v 1.11 2011-02-21 07:35:54 sasa Exp $*/
/*05.03.2008	03.03.2008	Белых А.И.	l_uplpod_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplpod_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_uplpod_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_uplpod_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_uplpod_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_uplpod_v_knopka),&data);
...
//g_print("l_uplpod_v_knopka knop=%d\n",knop);
...
//g_print("l_uplpod_v_vvod enter=%d\n",enter);
...
sprintf(strsql,"update Upldok set kp=%d where kp=%d",
atoi(nkod),atoi(skod));
...
sprintf(strsql,"update Upldok1 set kp=%d where kp=%d",
atoi(nkod),atoi(skod));

iceb_sql_zapis(strsql,1,0,wpredok);

sprintf(strsql,"update Upldok2 set kp=%d where kp=%d",
atoi(nkod),atoi(skod));
