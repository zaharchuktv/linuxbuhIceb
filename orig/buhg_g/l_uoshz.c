/*$Id: l_uoshz.c,v 1.15 2011-02-21 07:35:54 sasa Exp $*/
/*12.11.2009	18.10.2007	Белых А.И.	l_uoshz.c
...
sprintf(bros,"%s %s",name_system,gettext("Список шифров затрат"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uoshz_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_uoshz_knopka),&data);
...
{
iceb_u_str repl;
//printf("l_uoshz_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
class l_uoshz_data *data)
{
//printf("l_uoshz_v_row\n");
//printf("l_uoshz_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
iceb_u_zagolov(gettext("Список шифров затрат"),0,0,0,0,0,0,organ,ff);
