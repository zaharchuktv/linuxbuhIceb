/*$Id: l_sptbn.c,v 1.22 2011-02-21 07:35:53 sasa Exp $*/
/*23.04.2010	14.09.2006	Белых А.И.	l_sptbn.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(sptbn_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
tooltips[SFK2]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips[SFK2],data.knopka[SFK2],gettext("Просмотр карточки работника"),NULL);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK6]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK7]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK7]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK8]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
tooltips[FK8]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips[FK8],data.knopka[FK8],gettext("Включить/выключить расчёт сальдо"),NULL);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK9]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(sptbn_knopka),&data);
...
{
iceb_u_str repl;
//printf("sptbn_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(data->metka_saldo == 1)
   {
    double saldb=0.;
    sprintf(strsql,"%.2f",zarsaldw(1,data->ms,data->gs,atoi(row[0]),&saldb,data->window));
    ss[COL_SALDO].new_plus(strsql);
   }
  else
  ...
  COL_TABNOM,ss[COL_TABNOM].ravno_atoi(),
  COL_FIO,ss[COL_FIO].ravno(),
  COL_SALDO,ss[COL_SALDO].ravno_atof(),
...
{
//printf("sptbn_v_row\n");
//printf("sptbn_v_row корректировка\n");
