/*$Id: l_uplscnt.c,v 1.12 2011-02-21 07:35:54 sasa Exp $*/
/*12.11.2009	10.03.2008	Белых А.И.	l_uplscnt.c
...
sprintf(bros,"%s %s",name_system,gettext("Справочник средних цен на топливо"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplscnt_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_uplscnt_knopka),&data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Код топлива
  ss[COL_KOD_TOP].new_plus(iceb_u_toutf(row[1]));

  //Цена
  ss[COL_CENA].new_plus(iceb_u_toutf(row[2]));

  //Коеффициент перевода литров в киллограмы
  ss[COL_KOEF].new_plus(iceb_u_toutf(row[5]));
...
data->kod_topv.new_plus(iceb_u_fromutf(naim));
data->snanomer=nomer;
...
iceb_u_zagolov(gettext("Справочник средних цен на топливо"),0,0,0,0,0,0,organ,ff);
