/*$Id: l_zarud.c,v 1.16 2011-02-21 07:35:55 sasa Exp $*/
/*14.11.2009	23.06.2006	Белых А.И.	l_zarud.c
...
sprintf(bros,"%s %s",name_system,gettext("Список удержаний"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_zarud_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_zarud_knopka),&data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Счёт
  ss[COL_SHETB].new_plus(iceb_u_toutf(row[6]));
...
class l_zarud_data *data)
{
//printf("l_zarud_v_row\n");
//printf("l_zarud_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
iceb_u_zagolov(gettext("Список удержаний"),0,0,0,0,0,0,organ,ff);
...
fprintf(ff,gettext("\
Код |Наименование        | Счёт  |Проводки|Дата и время запис.| Кто записал\n"));
