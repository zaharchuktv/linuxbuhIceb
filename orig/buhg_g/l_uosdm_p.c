/*$Id: l_uosdm_p.c,v 1.6 2011-02-21 07:35:53 sasa Exp $*/
/*01.03.2009	01.03.2009	Белых А.И.	l_uosdm_p.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uosdm_p_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_uosdm_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_uosdm_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK2]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK4]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK10]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_p_knopka),&data);
...
//g_print("l_uosdm_p_knopka knop=%d\n",knop);
...
//g_print("l_uosdm_p_vvod enter=%d\n",enter);
