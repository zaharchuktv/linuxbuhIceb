/*$Id: l_uosamort_v.c,v 1.10 2011-02-21 07:35:53 sasa Exp $*/
/*11.12.2007	11.12.2007	Белых А.И.	l_uosamort_v.c
...
int l_uosamort_v(int innom,int podr,const char *mes_god,const char *hzt,const char *hna,GtkWidget *wpredok)
{
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uosamort_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATA]),"clicked",GTK_SIGNAL_FUNC(l_uosamort_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATA]), "activate",GTK_SIGNAL_FUNC(l_uosamort_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_SUMA]), "activate",GTK_SIGNAL_FUNC(l_uosamort_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK2]),"clicked",GTK_SIGNAL_FUNC(l_uosamort_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK10]),"clicked",GTK_SIGNAL_FUNC(l_uosamort_v_knopka),&data);
...
int knop=GPOINTER_TO_INT(gtk_object_get_user_data(GTK_OBJECT(widget)));
//g_print("l_uosamort_v_knopka knop=%d\n",knop);
...
//g_print("l_uosamort_v_vvod enter=%d\n",enter);
