/*$Id: l_uplavt.c,v 1.11 2011-02-21 07:35:54 sasa Exp $*/
/*12.11.2009	06.03.2008	Белых А.И.	l_uplavt.c
...
gboolean   l_uplavt_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplavt_data *data);
void l_uplavt_vibor(GtkTreeSelection *selection,class l_uplavt_data *data);
void l_uplavt_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uplavt_data *data);
void  l_uplavt_knopka(GtkWidget *widget,class l_uplavt_data *data);
void l_uplavt_add_columns (GtkTreeView *treeview);
void l_uplavt_udzap(class l_uplavt_data *data);
int  l_uplavt_prov_row(SQL_str row,class l_uplavt_data *data);
void l_uplavt_rasp(class l_uplavt_data *data);
int  l_uplavt_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uplavt_create_list (class l_uplavt_data *data);

int l_uplavt_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uplavt_p(class l_uplavt_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uplavt_data *data);
...
int   l_uplavt(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",name_system,gettext("Список автомобилей"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplavt_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_uplavt_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uplavt_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uplavt_vibor),data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uplavt_prov_row(row,data) != 0)
    continue;
  ...
  /*Гос-номер*/
  ss[COL_GOS_NOMER].new_plus(iceb_u_toutf(row[1]));

  /*Летняя норма затрта в городе*/
  ss[COL_LNZ_G].new_plus(row[2]);
  /*Зимняя норма затрта в городе*/
  ss[COL_ZNZ_G].new_plus(row[3]);
  ...
  /*Летняя норма затрта в городе*/
  ss[COL_LNZ_ZG].new_plus(row[8]);
  /*Зимняя норма затрта в городе*/
  ss[COL_ZNZ_ZG].new_plus(row[9]);

  /*норма затрта на перевозку груза*/
  ss[COL_NZ_GRUZ].new_plus(row[10]);

  /*норма затрта на отработанное время*/
  ss[COL_NZ_OV].new_plus(row[11]);
  
  //Наименование
  ss[COL_NAIM].new_plus(iceb_u_toutf(row[5]));
  ...
  COL_GOS_NOMER,ss[COL_GOS_NOMER].ravno(),
  COL_LNZ_G,ss[COL_LNZ_G].ravno(),
  COL_ZNZ_G,ss[COL_ZNZ_G].ravno(),
  COL_LNZ_ZG,ss[COL_LNZ_ZG].ravno(),
  COL_ZNZ_ZG,ss[COL_ZNZ_ZG].ravno(),
  COL_NZ_GRUZ,ss[COL_NZ_GRUZ].ravno(),
  COL_NZ_OV,ss[COL_NZ_OV].ravno(),
...
l_uplavt_add_columns (GTK_TREE_VIEW (data->treeview));
...
renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Летняя норма затрат в городе"), renderer,"text", COL_LNZ_G,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Зимняя норма затрат в городе"), renderer,"text", COL_ZNZ_G,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Летняя норма затрат за городом"), renderer,"text", COL_LNZ_ZG,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Зимняя норма затрат за городом"), renderer,"text", COL_ZNZ_ZG,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Норма затрат на перевозку груза"), renderer,"text", COL_NZ_GRUZ,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Норма затрат на отработанное время"), renderer,"text", COL_NZ_OV,NULL);
...
//g_print("l_uplavt_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_uplavt_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uplavt_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uplavt_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uplavt_create_list(data);
...
class l_uplavt_data *data)
{
//printf("l_uplavt_v_row\n");
//printf("l_uplavt_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
if(l_uplavt_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;
...
iceb_u_zagolov(gettext("Список автомобилей"),0,0,0,0,0,0,organ,ff);
...
fprintf(ff,gettext("\
Код |    Наименование    |   Водитель         |Государственный     |Расход по городу |Расход за городом|Расход  |Расход  |   Дата и время    |Кто записал\n\
    |     автомобиля     |                    |     номер          | летний | зимний | летний | зимний |на груз |на время|     записи        |\n"));
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  memset(voditel,'\0',sizeof(voditel));

  sprintf(strsql,"select naik from Uplouot where kod=%d",atoi(row[4]));
  if(iceb_sql_readkey(strsql,&row1,&cur1,data->window) == 1)
    sprintf(voditel,"%s %s",row[4],row1[0]);
  ...
  fprintf(ff,"%-4s %-*.*s %-*.*s %-*.*s %8s %8s %8s %8s %8s %8s %s %s\n",
  row[0],
  iceb_u_kolbait(20,row[5]),iceb_u_kolbait(20,row[5]),row[5],
  iceb_u_kolbait(20,voditel),iceb_u_kolbait(20,voditel),voditel,
  iceb_u_kolbait(20,row[1]),iceb_u_kolbait(20,row[1]),row[1],
  row[2],row[3],row[8],row[9],row[10],row[11],s5.ravno(),s6.ravno());
...
int  l_uplavt_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uplavt_pvu(0,row[0],data->window) != 0)
   continue;
