/*$Id: l_poizardok.c,v 1.12 2011-02-21 07:35:53 sasa Exp $*/
/*27.07.2008	27.07.2008	Белых А.И.	l_poizardok.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(poizardok_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(poizardok_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(poizardok_knopka),&data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Дата документа
  ss[COL_DATADOK].new_plus(iceb_u_sqldata(row[1]));
...
COL_DATADOK,&datadok,COL_NOMDOK,&nomdok,NUM_COLUMNS,&nomer,-1);
...
//g_print("poizardok_knopka knop=%d\n",knop);
...
switch (knop)
 {
  case FK2:
    ...
    l_zar_dok_zap(data->tipz,data->datadok.ravno(),data->nomdok.ravno(),data->window);
...
{
iceb_u_str repl;
//printf("poizardok_key_press keyval=%d state=%d\n",event->keyval,event->state);
