/*$Id: l_ukrspdok.c,v 1.10 2011-02-21 07:35:53 sasa Exp $*/
/*21.02.2008	21.02.2008	Белых А.И.	l_ukrspdok.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(ukrspdok_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(ukrspdok_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(ukrspdok_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(ukrspdok_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(ukrspdok_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(ukrspdok_knopka),&data);
...
sprintf(bros,"select nomd,datd,kont,nompr,mprov,mpodt,vkom,nomao,datao,ktoz,vrem,datp from Ukrdok");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Номер документа
  ss[COL_NOMDOK].new_plus(iceb_u_toutf(row[0]));
  ...
  //номер приказа
  ss[COL_NOMER_PRIK].new_plus(iceb_u_toutf(row[3]));

  //номер авансового отчёта
  ss[COL_NOMER_AO].new_plus(iceb_u_toutf(row[7]));
  ...
  if(row[8][0] != '0')
   ...
   ss[COL_DATA_AO].new_plus(iceb_u_sqldata(row[8]));

  /*дата приказа*/
  if(row[11][0] == '0')
    ss[COL_DATA_PRIK].new_plus("");
  else   
    ss[COL_DATA_PRIK].new_plus(iceb_u_sqldata(row[11]));
  ...
  COL_DATA_PRIK,ss[COL_DATA_PRIK].ravno(),
  COL_DATA_AO,ss[COL_DATA_AO].ravno(),
...
if(iceb_u_proverka(data->poi.vidkom.ravno(),row[6],0,0) != 0)
 return(1);
...
if(iceb_u_proverka(data->poi.nomer_prik.ravno(),row[3],0,0) != 0)
 return(1);

if(iceb_u_proverka(data->poi.nomer_ao.ravno(),row[7],0,0) != 0)
 return(1);
