/*$Id: l_uplgo_v.c,v 1.11 2011-02-21 07:35:54 sasa Exp $*/
/*09.03.2008	09.03.2008	Белых А.И.	l_uplgo_v.c
Ввод и корректировка групп объектов списания топлива
*/
#include <stdlib.h>
#include <unistd.h>
#include <time.h>
...
class l_uplgo_v_data
 {
  ...
  class l_uplgo_rek rk;
  class iceb_u_str kod_gr;  
  
  l_uplgo_v_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
gboolean   l_uplgo_v_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplgo_v_data *data);
void  l_uplgo_v_knopka(GtkWidget *widget,class l_uplgo_v_data *data);
void    l_uplgo_v_vvod(GtkWidget *widget,class l_uplgo_v_data *data);
int l_uplgo_pk(const char *god,GtkWidget*);
int l_uplgo_zap(class l_uplgo_v_data *data);
void l_uplgo_kkvt(const char *kods,const char *kodn,GtkWidget *wpredok);
...
if(data.kod_gr.getdlinna() >  1)
 {
  ...
  sprintf(strsql,"select * from Uplgost where kod=%d",data.kod_gr.ravno_atoi());
  SQL_str row;
  SQLCURSOR cur;
 ...
 {
  sprintf(strsql,"%d",iceb_get_new_kod("Uplgost",0,wpredok));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplgo_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_uplgo_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_uplgo_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_uplgo_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_uplgo_v_knopka),&data);
...
//g_print("l_uplgo_v_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    data->read_rek();
    if(l_uplgo_zap(data) == 0)
     {
      data->voz=0;
...
//g_print("l_uplgo_v_vvod enter=%d\n",enter);
...
if(iceb_u_SRAV(data->kod_gr.ravno(),data->rk.kod.ravno(),0) != 0)
  if(l_uplgo_pk(data->rk.kod.ravno(),data->window) != 0)
     return(1);

if(data->kod_gr.getdlinna() <= 1)
 {

  sprintf(strsql,"insert into Uplgost \
values (%d,'%s',%d,%ld)",
...
if(metkakor == 1)
 l_uplgo_kkvt(data->kod_gr.ravno(),data->rk.kod.ravno(),data->window);
...
/**********************************/
int l_uplgo_pk(const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kod from Uplgost where kod=%d",atoi(kod));
//printf("l_uplgo_pk-%s\n",strsql);
...
void l_uplgo_kkvt(const char *skod,const char *nkod,GtkWidget *wpredok)
{

char strsql[512];

sprintf(strsql,"update Uplost set kgr=%d where kgr=%d",
atoi(nkod),atoi(skod));
