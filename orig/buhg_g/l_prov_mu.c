/*$Id: l_prov_mu.c,v 1.39 2011-02-21 07:35:53 sasa Exp $*/
/*18.06.2010	18.08.2004	Белых А.И.	l_prov_mu.c
...
void       prov_mu_radio1(GtkWidget *,class prov_mu_data *);
void       prov_mu_radio2(GtkWidget *,class prov_mu_data *);
...
data.sklad=sklad;
strcpy(data.metkasys,gettext("МУ"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(prov_mu_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[0]), "clicked",GTK_SIGNAL_FUNC(prov_mu_radio0),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[1]), "clicked",GTK_SIGNAL_FUNC(prov_mu_radio1),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton[2]), "clicked",GTK_SIGNAL_FUNC(prov_mu_radio2),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK4]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK5]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK6]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK9]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(prov_mu_knopka),&data);
...
datd='%04d-%02d-%02d' and oper='%s' and tz=%d order by datp asc",
  data->metkasys,data->sklad,data->nomdok.ravno_filtr(),
...
switch (knop)
 {
  ...
  case FK2:
    ...
    if(iceb_vprov(data->metkasys,"","","","","",data->nomdok.ravno(),data->kodop.ravno(),data->sklad,
    data->dd,data->md,data->gd,data->kontr.ravno(),data->vrem_v,data->ktoz_v,mtsh,
    1,data->tipz,0,0,data->window) != 0)
         return;  
  ...
  case SFK2:
    ...
    data->dd,data->md,data->gd,data->kontr.ravno(),data->vrem_v,data->ktoz_v,mtsh,
    1,data->tipz,data->val_v,data->kekv,data->window) != 0)
         return;  
  ...
  case SFK3: 
    ...
    if(iceb_udprgr(data->metkasys,data->dd,data->md,data->gd,data->nomdok.ravno(),data->sklad,data->tipz,data->window) != 0)
     return;
  ...
  case FK10:
    ...
    prosprw(0,data->sklad,data->dd,data->md,data->gd,data->nomdok.ravno(),
    data->tipz,data->lnds,data->sklad1,data->nomdokp.ravno(),data->kodop.ravno(),NULL,data->window);
...
{
iceb_u_str repl;
printf("prov_mu_key_press keyval=%d state=%d\n",
...
void  prov_mu_radio1(GtkWidget *widget,class prov_mu_data *data)
{
//g_print("prov_mu_radio1\n");
if(data->metka_r == 1)
  return;
data->metka_r=1;
prov_mu_create_list(data);
}
void  prov_mu_radio2(GtkWidget *widget,class prov_mu_data *data)
{
//g_print("prov_mu_radio1\n");
if(data->metka_r == 2)
  return;
