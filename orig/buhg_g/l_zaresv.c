/*$Id: l_zaresv.c,v 1.1 2011-02-21 07:38:50 sasa Exp $*/
/*23.01.2011	23.01.2011	Белых А.И.	l_zaresv.c
...
sprintf(strsql,"%s %s",name_system,gettext("Список процентов единого социального взноса"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_zaresv_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_zaresv_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_zaresv_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_zaresv_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_zaresv_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_zaresv_knopka),&data);
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  //дата
  ss[COL_DATAND].new_plus(iceb_u_sqldata(row[1]));
  
  //ПРОЦЕНТ
  ss[COL_PRF].new_plus(row[2]);


  //ПРОЦЕНТ
  ss[COL_PRR].new_plus(row[3]);
  ...
  COL_DATAND,ss[COL_DATAND].ravno(),
  COL_PRF,ss[COL_PRF].ravno(),
  COL_PRR,ss[COL_PRR].ravno(),
...
data->datand.new_plus(iceb_u_fromutf(datand));
data->snanomer=nomer;
...
iceb_u_zagolov(gettext("Список процентов единого социального взноса"),0,0,0,0,0,0,organ,ff);
