/*$Id: l_xdkop.c,v 1.22 2011-02-21 07:35:54 sasa Exp $*/
/*08.06.2006	18.01.2006	Белых А.И.	l_xdkop.c
...
if(iceb_u_SRAV("Pltp",tablica,0) == 0)
 {
...
if(iceb_u_SRAV("Pltt",tablica,0) == 0)
 {
...
sprintf(bros,"%s %s",name_system,data.naim_spis.ravno_toutf());
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(xdkop_key_press),&data);
...
data.label_kolstr=gtk_label_new (data.naim_spis.ravno_toutf());
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(xdkop_knopka),&data);
...
{
iceb_u_str repl;
//printf("xdkop_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
{
//printf("xdkop_v_row\n");
//printf("xdkop_v_row корректировка\n");
...
iceb_u_zagolov(data->naim_spis.ravno(),0,0,0,0,0,0,organ,ff);
