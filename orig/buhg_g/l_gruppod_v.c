/*$Id: l_gruppod_v.c,v 1.23 2011-02-21 07:35:52 sasa Exp $*/
/*05.05.2008	21.06.2006	Белых А.И.	l_gruppod_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_gruppod_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_gruppod_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_gruppod_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_gruppod_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_gruppod_v_knopka),&data);
...
//g_print("l_gruppod_v_knopka knop=%d\n",knop);
...
//g_print("l_gruppod_v_vvod enter=%d\n",enter);
...
sprintf(strsql,"update Podr set grup='%s' where grup='%s'",
nkod,skod);
