/*$Id: arz_m.c,v 1.12 2011-02-21 07:35:51 sasa Exp $*/
/*23.11.2006	08.11.2006	Белых А.И.	arz_m.c
  ...
  void read_rek()
   {
    ...
    rk->data_per.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_DATA_PER]))));
    rk->max_sum.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_MAX_SUM]))));
   }
...
sprintf(strsql,"%s %s",name_system,gettext("Автоматическая разноска зарплаты"));
label=gtk_label_new(gettext("Автоматическая разноска зарплаты"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
gtk_signal_connect(GTK_OBJECT(data.window),"delete_event",GTK_SIGNAL_FUNC(gtk_widget_destroy),NULL);
gtk_signal_connect(GTK_OBJECT(data.window),"destroy",GTK_SIGNAL_FUNC(gtk_main_quit),NULL);
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(arz_m_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton2[0]), "clicked",GTK_SIGNAL_FUNC(arz_radio2_0),&data);
...
gtk_signal_connect(GTK_OBJECT(data.radiobutton2[1]), "clicked",GTK_SIGNAL_FUNC(arz_radio2_1),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATA]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATA]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_TABNOM]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_TABNOM]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
data.knopka_enter[E_DATA_PER]=gtk_button_new_with_label(iceb_u_toutf(strsql));
gtk_box_pack_start (GTK_BOX (hbox[E_DATA_PER]), data.knopka_enter[E_DATA_PER], FALSE, FALSE, 0);
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATA_PER]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
gtk_object_set_user_data(GTK_OBJECT(data.knopka_enter[E_DATA_PER]),(gpointer)E_DATA_PER);
tooltips_enter[E_DATA_PER]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips_enter[E_DATA_PER],data.knopka_enter[E_DATA_PER],gettext("Выбор даты"),NULL);

data.entry[E_DATA_PER] = gtk_entry_new_with_max_length (10);
gtk_box_pack_start (GTK_BOX (hbox[E_DATA_PER]), data.entry[E_DATA_PER], FALSE, FALSE, 0);
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATA_PER]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_DATA_PER]),data.rk->data_per.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_DATA_PER]),(gpointer)E_DATA_PER);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KOD_NAH]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD_NAH]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_PODR]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_PODR]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KOD_GRUP_POD]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD_GRUP_POD]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_KOD_KAT]),"clicked",GTK_SIGNAL_FUNC(arz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD_KAT]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_PROC_VIP]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
...
data.entry[E_MAX_SUM] = gtk_entry_new_with_max_length (10);
gtk_box_pack_start (GTK_BOX (hbox[E_MAX_SUM]), data.entry[E_MAX_SUM], TRUE, TRUE, 0);
gtk_signal_connect(GTK_OBJECT (data.entry[E_MAX_SUM]), "activate",GTK_SIGNAL_FUNC(arz_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_MAX_SUM]),data.rk->max_sum.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_MAX_SUM]),(gpointer)E_MAX_SUM);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(arz_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(arz_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(arz_m_knopka),&data);
...
switch (enter)
 {
  ...
  case E_TABNOM:
    ...
    break;
  case E_DATA_PER:
    data->rk->data_per.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
  case E_PODR:
  ...
  case E_PROC_VIP:
    ...
    break;
  case E_MAX_SUM:
    data->rk->max_sum.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
