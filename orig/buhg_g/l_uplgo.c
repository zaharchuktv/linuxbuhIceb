/*$Id: l_uplgo.c,v 1.11 2011-02-21 07:35:54 sasa Exp $*/
/*12.11.2009	09.03.2008	Белых А.И.	l_uplgo.c
Работа со списком групп объектов списания топлива
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gboolean   l_uplgo_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplgo_data *data);
void l_uplgo_vibor(GtkTreeSelection *selection,class l_uplgo_data *data);
void l_uplgo_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uplgo_data *data);
void  l_uplgo_knopka(GtkWidget *widget,class l_uplgo_data *data);
void l_uplgo_add_columns (GtkTreeView *treeview);
void l_uplgo_udzap(class l_uplgo_data *data);
int  l_uplgo_prov_row(SQL_str row,class l_uplgo_data *data);
void l_uplgo_rasp(class l_uplgo_data *data);
int  l_uplgo_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uplgo_create_list (class l_uplgo_data *data);

int l_uplgo_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uplgo_p(class l_uplgo_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uplgo_data *data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uplgo_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uplgo_vibor),data);
...
sprintf(strsql,"select * from Uplgost order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uplgo_prov_row(row,data) != 0)
    continue;
...
l_uplgo_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список групп объектов списания топлива"));
...
//g_print("l_uplgo_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_uplgo_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uplgo_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uplgo_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uplgo_create_list(data);
...
class l_uplgo_data *data)
{
//printf("l_uplgo_v_row\n");
//printf("l_uplgo_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
if(l_uplgo_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uplgost where kod=%d",
...
sprintf(strsql,"gost%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Список групп объектов списания топлива"));
...
iceb_u_zagolov(gettext("Список групп объектов списания топлива"),0,0,0,0,0,0,organ,ff);
...
int  l_uplgo_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kgr from Uplost where kgr=%d limit 1",atoi(kod));
...
char strsql[512];
sprintf(strsql,"select kod from Uplgost");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uplgo_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uplgost where kod=%d",atoi(row[0]));
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
