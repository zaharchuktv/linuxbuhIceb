/*$Id: l_uosdm.c,v 1.11 2011-02-21 07:35:53 sasa Exp $*/
/*12.11.2009	01.03.2009	Белых А.И.	l_uosdm.c
Работа со списком драгоценных металлов
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uosdm_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_uosdm_knopka),&data);
...
switch (knop)
 {
  case FK2:
    l_uosdm_v(&data->kod_kat_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uosdm_v(&data->kodv,data->window);
...
class l_uosdm_data *data)
{
//printf("l_uosdm_v_row\n");
//printf("l_uosdm_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
