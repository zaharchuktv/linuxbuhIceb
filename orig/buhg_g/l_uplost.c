/*$Id: l_uplost.c,v 1.11 2011-02-21 07:35:54 sasa Exp $*/
/*12.11.2009	09.03.2008	Белых А.И.	l_uplost.c
Работа со списком объектов списания топлива
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gboolean   l_uplost_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplost_data *data);
void l_uplost_vibor(GtkTreeSelection *selection,class l_uplost_data *data);
void l_uplost_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uplost_data *data);
void  l_uplost_knopka(GtkWidget *widget,class l_uplost_data *data);
void l_uplost_add_columns (GtkTreeView *treeview);
void l_uplost_udzap(class l_uplost_data *data);
int  l_uplost_prov_row(SQL_str row,class l_uplost_data *data);
void l_uplost_rasp(class l_uplost_data *data);
int  l_uplost_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uplost_create_list (class l_uplost_data *data);

int l_uplost_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uplost_p(class l_uplost_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uplost_data *data);
...
int   l_uplost(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",name_system,gettext("Список объектов списания топлива"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplost_key_press),&data);
...
data.label_kolstr=gtk_label_new (gettext("Список объектов списания топлива"));
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK3]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(l_uplost_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uplost_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uplost_vibor),data);
...
sprintf(strsql,"select * from Uplost order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uplost_prov_row(row,data) != 0)
    continue;
  ...
  /*Код группы*/
  ss[COL_KGR].new_plus(iceb_u_toutf(row[1]));
...
l_uplost_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список объектов списания топлива"));
...
renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Группа"), renderer,"text", COL_KGR,NULL);
...
//g_print("l_uplost_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_uplost_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uplost_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uplost_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uplost_create_list(data);
...
class l_uplost_data *data)
{
//printf("l_uplost_v_row\n");
//printf("l_uplost_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  gtk_signal_emit_by_name(GTK_OBJECT(data->knopka[SFK2]),"clicked");
  return;
...
if(l_uplost_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uplost where kod=%d",data->kodv.ravno_atoi());
...
if(data->poisk.naim.ravno()[0] != '\0' && iceb_u_strstrm(row[2],data->poisk.naim.ravno()) == 0)
 return(1);
...
imaf.plus(strsql);
naimot.plus(gettext("Список объектов списания топлива"));
...
iceb_u_zagolov(gettext("Список объектов списания топлива"),0,0,0,0,0,0,organ,ff);
...
int  l_uplost_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select ko from Upldok2 where ko=%d limit 1",atoi(kod));
...
char strsql[512];
sprintf(strsql,"select kod from Uplost");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uplost_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uplost where kod=%d",atoi(row[0]));
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
