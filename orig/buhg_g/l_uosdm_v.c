/*$Id: l_uosdm_v.c,v 1.9 2011-02-21 07:35:53 sasa Exp $*/
/*01.03.2009	21.06.2006	Белых А.И.	l_uosdm_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uosdm_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_uosdm_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_uosdm_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_EI]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_v_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_EI]), "activate",GTK_SIGNAL_FUNC(l_uosdm_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK2]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK10]),"clicked",GTK_SIGNAL_FUNC(l_uosdm_v_knopka),&data);
...
int knop=GPOINTER_TO_INT(gtk_object_get_user_data(GTK_OBJECT(widget)));
//g_print("l_uosdm_v_knopka knop=%d\n",knop);
...
//g_print("l_uosdm_v_vvod enter=%d\n",enter);
