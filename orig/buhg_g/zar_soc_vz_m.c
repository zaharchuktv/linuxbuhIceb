/*$Id: zar_soc_vz_m.c,v 1.10 2011-02-21 07:35:59 sasa Exp $*/
/*16.01.2007	16.01.2007	Белых А.И.	zar_soc_vz_m.c
  ...
  void read_rek()
   {
    ...
    rk->vs_mes.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(entry[E_VS_MES]))));
...
sprintf(strsql,"%s %s",name_system,gettext("Распечатать расчёт отчислений на выданную зарплату"));
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(zar_soc_vz_m_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAN]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAN]), "activate",GTK_SIGNAL_FUNC(zar_soc_vz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_DATAK]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_DATAK]), "activate",GTK_SIGNAL_FUNC(zar_soc_vz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_TABNOM]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_TABNOM]), "activate",GTK_SIGNAL_FUNC(zar_soc_vz_m_vvod),&data);
...
data.knopka_enter[E_VS_MES]=gtk_button_new_with_label(iceb_u_toutf(strsql));
gtk_box_pack_start (GTK_BOX (hbox[E_VS_MES]), data.knopka_enter[E_VS_MES], FALSE, FALSE, 0);
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_VS_MES]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_e_knopka),&data);
gtk_object_set_user_data(GTK_OBJECT(data.knopka_enter[E_VS_MES]),(gpointer)E_VS_MES);
tooltips_enter[E_VS_MES]=gtk_tooltips_new();
gtk_tooltips_set_tip(tooltips_enter[E_VS_MES],data.knopka_enter[E_VS_MES],gettext("Выбор даты"),NULL);

data.entry[E_VS_MES] = gtk_entry_new_with_max_length(7);
gtk_box_pack_start (GTK_BOX (hbox[E_VS_MES]), data.entry[E_VS_MES], FALSE, FALSE, 0);
gtk_signal_connect(GTK_OBJECT (data.entry[E_VS_MES]), "activate",GTK_SIGNAL_FUNC(zar_soc_vz_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_VS_MES]),data.rk->vs_mes.ravno_toutf());
gtk_object_set_user_data(GTK_OBJECT(data.entry[E_VS_MES]),(gpointer)E_VS_MES);
...
gtk_signal_connect(GTK_OBJECT(data.knopka_enter[E_PODR]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_e_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_PODR]), "activate",GTK_SIGNAL_FUNC(zar_soc_vz_m_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(zar_soc_vz_m_knopka),&data);
...
switch (enter)
 {
  ...
  case E_PODR:
    ...
    break;
  case E_VS_MES:
    data->rk->vs_mes.new_plus(iceb_u_fromutf((char*)gtk_entry_get_text(GTK_ENTRY(widget))));
    break;
