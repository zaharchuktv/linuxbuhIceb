/*$Id: l_zarkateg_v.c,v 1.22 2011-02-21 07:35:55 sasa Exp $*/
/*16.09.2010	21.06.2006	Белых А.И.	l_zarkateg_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_zarkateg_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_zarkateg_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_zarkateg_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_zarkateg_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(l_zarkateg_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_zarkateg_v_knopka),&data);
...
//g_print("l_zarkateg_v_knopka knop=%d\n",knop);
...
//g_print("l_zarkateg_v_vvod enter=%d\n",enter);
