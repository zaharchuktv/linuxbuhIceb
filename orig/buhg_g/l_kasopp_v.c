/*$Id: l_kasopp_v.c,v 1.31 2011-02-21 07:35:52 sasa Exp $*/
/*01.06.2009	18.01.2006	Белых А.И.	l_kasopp_v.c
...
//g_print("l_kasopp_v_v_vvod enter=%d\n",enter);
...
sprintf(strsql,"update Prov set oper='%s' where oper='%s' and tz=1 and kto='%s'",nkod,skod,gettext("КО"));
if(sql_zap(&bd,strsql) != 0)
  iceb_msql_error(&bd,gettext("Ошибка корректировки записи !"),strsql,wpredok);
