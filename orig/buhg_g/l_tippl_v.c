/*$Id: l_tippl_v.c,v 1.13 2011-02-21 07:35:53 sasa Exp $*/
/*12.05.2006	12.05.2006	Белых А.И.	l_tipplp_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_tipplp_v_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_tipplp_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_tipplp_v_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_tipplp_v_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_tipplp_v_v_knopka),&data);
...
//g_print("l_tipplp_v_v_vvod enter=%d\n",enter);
...
if(iceb_u_SRAV(tablica,"Ktnp",0) == 0)
  sprintf(strsql,"update Tpltpz set nomd='%s' where nomd='%s'",nkod,skod);
if(iceb_u_SRAV(tablica,"Ktnt",0) == 0)
  sprintf(strsql,"update Tplttz set nomd='%s' where nomd='%s'",nkod,skod);
