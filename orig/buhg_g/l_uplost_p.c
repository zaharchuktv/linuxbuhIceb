/*$Id: l_uplost_p.c,v 1.6 2011-02-21 07:35:54 sasa Exp $*/
/*09.03.2008	09.03.2008	Белых А.И.	l_uplost_p.c
...
void l_uplost_p_clear(class l_uplost_p_data *data);
void    l_uplost_p_vvod(GtkWidget *widget,class l_uplost_p_data *data);
void  l_uplost_p_knopka(GtkWidget *widget,class l_uplost_p_data *data);
gboolean   l_uplost_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplost_p_data *data);

extern char      *name_system;

int l_uplost_p(class l_uplost_rek *rek_poi,GtkWidget *wpredok)
{
class l_uplost_p_data data;
char strsql[512];
data.rk=rek_poi;
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(l_uplost_p_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(l_uplost_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(l_uplost_p_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]),"clicked",GTK_SIGNAL_FUNC(l_uplost_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]),"clicked",GTK_SIGNAL_FUNC(l_uplost_p_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]),"clicked",GTK_SIGNAL_FUNC(l_uplost_p_knopka),&data);
...
//g_print("l_uplost_p_knopka knop=%d\n",knop);
...
//g_print("l_uplost_p_vvod enter=%d\n",enter);
