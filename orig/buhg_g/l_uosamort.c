/*$Id: l_uosamort.c,v 1.17 2011-02-21 07:35:53 sasa Exp $*/
/*12.11.2009	17.07.2005	Белых А.И.	l_uosamort.c
...
sprintf(strsql,"select * from Uosamor where innom=%d and god >= %d \
...
sprintf(strsql,"delete from Uosamor where innom=%d and god=%d \
...
iceb_u_zagolov(gettext("Амортизационные отчисления налогового учёта"),0,0,0,0,0,0,organ,ff);
...
if(data->innom > 0)
...
else
 {
  ...
  amort23w(data->innom,data->podr,kvrt,g1,data->hzt.ravno(),ff,data->window);
  fclose(ff);
  iceb_ustpeh(data->imafprot_ao.ravno(),3,data->window);
 } 
}
