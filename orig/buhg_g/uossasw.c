/* $Id: uossasw.c,v 1.10 2011-02-21 07:35:58 sasa Exp $ */
/*17.11.2009    29.05.1997      Белых А.И.      uossasw.c
...
if(iceb_alxout("uossas.alx",wpredok) != 0)
 return;
if((ff = fopen("uossas.alx","r")) == NULL)
 {
  iceb_er_op_fil("uossas.alx","",errno,wpredok);
  return;
 }
...
memset(str,'\0',sizeof(str));
fgets(str,sizeof(str),ff);
iceb_u_bstab(str,ndk,149,160,1);
iceb_u_bstab(str,datpr,161,171,1);
iceb_u_bstab(str,kprr,173,181,1);
fprintf(ff1,"%s",str);
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  iz=bal_st.siz+bal_st.iz+bal_st.iz;
...
memset(str,'\0',sizeof(str));
fgets(str,sizeof(str),ff);
iceb_u_bstab(str,naim,53,73,1);
iceb_u_bstab(str,model,90,110,1);
fprintf(ff1,"%s",str);
...
memset(str,'\0',sizeof(str));
fgets(str,sizeof(str),ff);
iceb_u_bstab(str,rekin.nomz.ravno(),55,65,1);
fprintf(ff1,"%s",str);
i=0;
for(;;)
 {
