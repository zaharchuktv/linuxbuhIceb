...
item = gtk_menu_item_new_with_label (iceb_u_toutf(gettext("0% НДС реализация на територии Украины")));
gtk_signal_connect (GTK_OBJECT (item), "activate",GtkSignalFunc(iceb_pm_nds_get),nds);
gtk_object_set_user_data(GTK_OBJECT(item),(gpointer)nomer++);
gtk_widget_show (item);
gtk_menu_append (GTK_MENU (menu), item);
...
item = gtk_menu_item_new_with_label (iceb_u_toutf(gettext("Освобождение от НДС статья 5")));
gtk_signal_connect (GTK_OBJECT (item), "activate",GtkSignalFunc(iceb_pm_nds_get),nds);
gtk_object_set_user_data(GTK_OBJECT(item),(gpointer)nomer++);
gtk_widget_show (item);
gtk_menu_append (GTK_MENU (menu), item);
