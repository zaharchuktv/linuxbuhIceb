/*$Id: iceb_l_gkontr_v.c,v 1.21 2011-01-13 13:49:59 sasa Exp $*/
/*05.05.2010	31.12.2003	Белых А.И.	iceb_l_gkontr_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(gkontr_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(gkontr_v_rk),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(gkontr_v_rk),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK2]),"clicked",GTK_SIGNAL_FUNC(gkontr_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK10]),"clicked",GTK_SIGNAL_FUNC(gkontr_v_knopka),&data);
...
//g_print("gkontr_v_knopka knop=%d\n",knop);
