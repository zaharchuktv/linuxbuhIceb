/*$Id: l_saldo_shk.c,v 1.25 2011-02-21 07:35:53 sasa Exp $*/
/*12.11.2009	30.12.2003	Белых А.И.	l_saldo_shk.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(saldo_shk_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK2]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[SFK2]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK3]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK4]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK5]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[FK10]), "clicked",GTK_SIGNAL_FUNC(saldo_shk_knopka),&data);
...
//printf("saldo_shk_create_list %d\n",data->snanomer);
...
sprintf(strsql,"select gs,ns,kodkon,deb,kre,ktoi,vrem from Saldo where kkk=1 order by gs asc, ns asc,kodkon asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Кредит
  s5.new_plus(iceb_u_toutf(row[4]));
  kredit+=atof(row[4]);

  sprintf(strsql,"select naikon from Kontragent where kodkon='%s'",row[2]);
  if(sql_readkey(&bd,strsql,&row1,&cur1) == 1)
   s6.new_plus(iceb_u_toutf(row1[0]));
  else
   s6.new_plus("");
  
  //Дата и время записи
  s7.new_plus(iceb_u_toutf(iceb_u_vremzap(row[6])));

  //Кто записал
  s8.new_plus(iceb_kszap(row[5],0,data->window));
...
if(data->poisk.metka_poi == 1)
 {
  ...
  if(data->poisk.god.getdlinna() > 1)  
   {
    stroka.new_plus(gettext("Год"));
    stroka.plus(": ");
    stroka.plus(data->poisk.god.ravno());
  ...
  if(data->poisk.shet.getdlinna() > 1)  
   {
    stroka.new_plus(gettext("Счет"));
    stroka.plus(": ");
    stroka.plus(data->poisk.shet.ravno());
    zagolov.ps_plus(stroka.ravno());
   }

  if(data->poisk.kodkontr.getdlinna() > 1)  
   {
    stroka.new_plus(gettext("Код контрагента"));
    stroka.plus(": ");
  ...
  if(data->poisk.debet.getdlinna() > 1)  
   {
    stroka.new_plus(gettext("Дебет"));
    stroka.plus(": ");
    stroka.plus(data->poisk.debet.ravno());
    zagolov.ps_plus(stroka.ravno());
   }

  if(data->poisk.kredit.getdlinna() > 1)  
   {
    stroka.new_plus(gettext("Кредит"));
    stroka.plus(": ");
    stroka.plus(data->poisk.kredit.ravno());
    zagolov.ps_plus(stroka.ravno());
   }
...
"text", COL_KTO,NULL);
printf("saldo_shk_add_columns end\n");
...
data->kodkontrv.new_plus(iceb_u_fromutf(kodkontr));
data->snanomer=nomer;
...
{
iceb_u_str repl;
printf("saldo_shk_key_press keyval=%d state=%d\n",
...
imaf.plus(strsql);
naimot.plus(gettext("Стартовые сальдо по счетам с развёрнутым сальдо"));
...
iceb_u_zagolov(gettext("Стартовые сальдо по счетам с развёрнутым сальдо"),0,0,0,0,0,0,organ,ff);
...
fprintf(ff,gettext("\
Год |Счет  |Код контр.| Дебет    | Кредит   |Дата и время запис.| Кто записал\n"));
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  fprintf(ff,"%s %-*s %-*s %10.2f %10.2f %s %s\n",
  row[0],
  iceb_u_kolbait(6,row[1]),row[1],
  iceb_u_kolbait(10,row[2]),row[2],
  atof(row[3]),atof(row[4]),s5.ravno(),s6.ravno());
