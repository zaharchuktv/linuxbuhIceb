...
12 datpnn дата получения счет-фактуры
...
3 nomerz=1 - доверенность#дата выдачи
        ...
        11 - НДС
             0 20% НДС.
             1 0% НДС реализация на територии России.
             2 0% НДС экспорт.
             3 0% Освобождение от НДС.
...
18 datpnn дата выдачи счет-фактуры
19 nomnn номер счет-фактуры
...
24 dvnn - дата выписки счет-фактуры
25 mo - метка оплаты 0- неоплачено 1-оплачено
26 vd - вида договора (для счет-фактуры)
*/
...
23 osnov  основание
24 datpnn дата получения счет-фактуры
...
12 kdrnn Код контрагента для реестра счет-фактур
13 ndrnn Номер документа, который в реестре нал. накладных будет записан как номер счет-фактур
14 sn    счёт НДС (Иногда нужно чтобы в одном документе разные записи шли на разные счета)
15 dvnn  дата выписки счет-фактур
...
/*111*/
iceb_pbar(data->bar,data->kolstr,++data->kolstr1);    
iceb_printw(iceb_u_toutf("Создание таблицы реестра выданных счет-фактур Reenn !\n"),data->buffer,data->view);
...
16 vd      вид документа
17 datv    дата выписки счет-фактуры
18 dnz     дневная нумерация записей
19 nnni    номер счет-фактуры в цифровой форме - для правильной сортрировки
***************/
if(sql_zap(&bd,strsql) != 0)
 { 
  sprintf(bros,"%d %s\n",sql_nerror(&bd),sql_error(&bd));
...
/*112*/
iceb_pbar(data->bar,data->kolstr,++data->kolstr1);    
iceb_printw(iceb_u_toutf("Создание таблицы реестра полученных счет-фактур Reenn1 !\n"),data->buffer,data->view);
...
/************
0  nz      уникальный номер записи устанавливаемый базой данных автоматически
1  datd    дата получения счет-фактуры
2  nnn     номер счет-фактуры
...
21 dvd     Дата выписки счет-фактуры
22 vd      вид документа
***************/
...
/*113*/
iceb_pbar(data->bar,data->kolstr,++data->kolstr1);    
iceb_printw(iceb_u_toutf("Создание таблицы групп полученных счет-фактур Reegrup1 !\n"),data->buffer,data->view);
...
/*114*/
iceb_pbar(data->bar,data->kolstr,++data->kolstr1);    
iceb_printw(iceb_u_toutf("Создание таблицы групп выданных счет-фактур Reegrup2 !\n"),data->buffer,data->view);
...
33 nnd    если равно 0 то разрешена работа с подсистемой "Реестр счет-фактур"
34 nnb    если равно 0 то разрешена блокировка подсистемы "Реестр счет-фактур"
35 nnn    если равно 0 то разрешена работа с настроечными файлами подсистемы "Реестр счет-фактур"
