...
char		*bnds1=NULL; /*0% НДС реализация на територии России.*/
char            *bnds2=NULL; /*0% НДС экспорт.*/
char            *bnds3=NULL; /*0% Освобождение от НДС.*/
...
while(cur_alx.read_cursor(&row_alx) != 0)
 {
  ...
  if(iceb_u_SRAV(st1,"0% НДС реализация на територии России",0) == 0)
    {
     memset(bros,'\0',sizeof(bros));
     iceb_u_polen(row_alx[0],bros,sizeof(bros),2,'|');
     i=strlen(bros)+1;
  ...
  if(iceb_u_SRAV(st1,"0% Освобождение от НДС",0) == 0)
    {
     memset(bros,'\0',sizeof(bros));
     iceb_u_polen(row_alx[0],bros,sizeof(bros),2,'|');
     i=strlen(bros)+1;
