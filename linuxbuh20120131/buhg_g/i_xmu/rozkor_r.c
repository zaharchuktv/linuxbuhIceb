...
if(sql_readkey(&bd,strsql,&row,&cur) != 1)
 {
  sprintf(strsql,"Не найден документ с номером счет-фактуры %s !",
  data->rk->nomer_nal_nak.ravno());
...
if(iceb_u_sravmydat(data->rk->dd,data->rk->md,data->rk->gd,di,mi,gi) < 0)
 {
  sprintf(strsql,"Дата счет-фактуры %d.%d.%d больше даты возврата %d.%d.%d !",
...
sprintf(strsql,"Номер накладной %s Номер счет-фактуры %s\n",
