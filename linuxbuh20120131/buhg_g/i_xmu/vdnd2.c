...
sprintf(strsql,"%s",gettext("Дата выдачи счет-фактуры"));
...
if(data->rk.nomnalnak.getdlinna() > 1 && data->rk.data_pnn.getdlinna() <= 1)
 {
  repl.new_plus(gettext("Не введена дата выдачи счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
...
if(iceb_u_rsdat(&d,&m,&g,data->rk.data_pnn.ravno(),0) != 0)
 {
  repl.new_plus(gettext("Не верно введена дата выдачи счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
 }
...
if(data->rk.data_pnn.prov_dat() != 0)
 {
  repl.new_plus(gettext("Не правильно введена дата выдачи счет-фактуры !"));
  iceb_menu_soob(&repl,data->window);
  return(1);  
 }
