...
char *str, //Строка
short gri)  //0-руб 1-рублей 2-только целую часть без руб.
{
char            s[512];
char            s1[512];
char            s2[512];
...
if(gri == 0)
  sprintf(sss,"%s %s %s %s",str,gettext("руб."),
  s2,gettext("коп."));
...
if(gri == 1)
 {
  ...
  strcpy(s1,gettext("рубль"));

  if(iceb_u_SRAV(s,gettext("одна"),0) == 0)
   strcpy(s1,gettext("рубль"));

  if(iceb_u_SRAV(s,gettext("два"),0) == 0)
    strcpy(s1,gettext("рубля"));
