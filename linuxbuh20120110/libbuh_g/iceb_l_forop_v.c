/*$Id: iceb_l_forop_v.c,v 1.19 2011-01-13 13:49:59 sasa Exp $*/
/*06.09.2010	10.05.2004	Белых А.И.	iceb_l_forop_v.c
...
gtk_signal_connect_after(GTK_OBJECT(data.window),"key_press_event",GTK_SIGNAL_FUNC(forop_v_key_press),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_KOD]), "activate",GTK_SIGNAL_FUNC(forop_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT (data.entry[E_NAIM]), "activate",GTK_SIGNAL_FUNC(forop_v_vvod),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK2]),"clicked",GTK_SIGNAL_FUNC(forop_v_knopka),&data);
...
gtk_signal_connect(GTK_OBJECT(data.knopka[PFK10]),"clicked",GTK_SIGNAL_FUNC(forop_v_knopka),&data);
...
sprintf(strsql,"update Usldokum set forop='%s' where forop='%s'",
nkod,skod);
if(sql_zap(&bd,strsql) != 0)
  iceb_msql_error(&bd,gettext("Ошибка корректировки записи !"),strsql,wpredok);
